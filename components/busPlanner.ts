...
export class BusPlannerService {
  ...
  public async getArrivalsBySlid(slid: string, stopName: string): Promise<any[]> {
    ...
    // 找到對應的 sid
    const sids = Object.keys(this.db.by_sid).filter(sid => {
      const info = this.db.by_sid[sid];
      return info?.slid === slid;
    });
    
    if (sids.length === 0) {
      console.warn(`[BusPlanner] No SID found for SLID: ${slid}`);
      return [];
    }
    
    // 使用第一個匹配的 sid
    const buses = await this.fetchBusesAtSid(sids[0]);
    
    // 排序並返回
    return buses.sort((a, b) => a.rawTime - b.rawTime);
  }
  ...
  public async getStopArrivals(stopName: string): Promise<any[]> {
    ...
    if (sids.length === 0) {
      console.warn(`[BusPlanner] No SIDs found for stop: ${stopName}`);
      return [];
    }
    ...
    return buses.map(bus => ({
      ...
      rid: bus.rid,
      sid: bus.sid,
      time_text: bus.timeText,
      arrivalTimeText: bus.timeText,
      raw_time: bus.rawTime,
      rawTime: bus.rawTime,
